...
{
	auto newIoContext = socketContext->GetNewIOContext();
	newIoContext->ioType = SEND_POSTED;
	newIoContext->ioSocket = socketContext->connSocket;
	// 投递recv请求
	if (false == PostSend(socketContext, newIoContext))
	{
		DoClose(socketContext);
		return false;
	}
	//newIoContext->Reset();
	return 0;
...
BOOL IOCPBase::DoRecv(SocketContext * sockContext, IOContext * ioContext)
{
	...
	SendData(sockContext, ioContext->wsaBuf.buf, ioContext->wsaBuf.len);
	ioContext->Reset();
